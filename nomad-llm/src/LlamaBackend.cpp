...
bool LlamaBackend::loadModel(const QString &modelPath, int nCtx, int nGpuLayers,
    ...
    if (progressCallback) {
        // Store callback globally or thread-locally if needed, but llama_model_params supports progress_callback
        // llama_model_params has: void (*progress_callback)(float progress, void * ctx)
        struct ProgressContext {
            std::function<void(float)> cb;
        };
        auto* progressCtx = new ProgressContext{progressCallback};
        ...
        model_params.progress_callback_user_data = progressCtx;
